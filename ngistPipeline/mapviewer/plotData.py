...
def plotMap(self, module, maptype):
    ...
    if module == "GAS":
        ...
        try:
            idx_AoNThreshold = np.where(
                self.gasResults[
                    maptype.split("_")[0] + "_" + maptype.split("_")[1] + "_AON"
                ][idxMap]
                < self.AoNThreshold
            )[0]
            ...
            print(
                "WARNING: No AoN threshold is applied to the displayed map of "
                + maptype
            )
    ...
    if module == "SFH":
        if maptype.split("_")[-1] == "DIFF":
            val = (
                self.kinResults[maptype.split("_")[0]][idxMap]
                - self.sfhResults[maptype.split("_")[0]][idxMap]
            )
        else:
    ...
    xmin = np.nanmin(self.table.X[idxMap]) - 1
    xmax = np.nanmax(self.table.X[idxMap]) + 1
    ymin = np.nanmin(self.table.Y[idxMap]) - 1
    ymax = np.nanmax(self.table.Y[idxMap]) + 1
...
def plotData(self):
    ...
    try:
        ...
        pass
    self.binMarker = self.axes[0].scatter(
        self.table.XBIN[self.idxBinLong],
        self.table.YBIN[self.idxBinLong],
        color=self.markercolor,
        marker="x",
    )
    self.spaxelMarker = self.axes[0].scatter(
        self.table.X[self.idxBinLong],
        self.table.Y[self.idxBinLong],
        color=self.markercolor,
        marker="o",
    )
    ...
    if self.KIN == False and self.GAS == False and self.SFH == False:
        self.plotPlainSpectrum(
            self.Spectra[self.idxBinShort], self.table.SNRBIN[self.idxBinLong], 1
        )
    ...
    if self.KIN == True:
        ...
        if (
            "V" in self.kinResults.names
            and "SIGMA" in self.kinResults.names
            and "H3" in self.kinResults.names
            ...
            self.axes[1].set_title(
                "Stellar kinematics: V={:.1f}km/s, SIGMA={:.1f}km/s, H3={:.2f}, H4={:.2f}".format(
                    ...
                    self.kinResults.H4[self.idxBinLong],
                ),
                loc="left",
            )
        elif "V" in self.kinResults.names and "SIGMA" in self.kinResults.names:
            self.axes[1].set_title(
                "Stellar kinematics: V={:.1f}km/s, SIGMA={:.1f}km/s".format(
                    self.kinResults.V[self.idxBinLong],
                    self.kinResults.SIGMA[self.idxBinLong],
                ),
                loc="left",
    ...
    if self.GAS == True:
        if self.gasLevel == "BIN":
            self.plotSpectraGAS(
                self.Spectra[self.idxBinShort],
                self.gasBestfit[self.idxBinShort],
                self.gasGoodpix,
                2,
            )
        elif self.gasLevel == "SPAXEL":
            self.plotSpectraGAS(
                self.AllSpectra[self.idxBinLong],
                self.gasBestfit[self.idxBinLong],
                self.gasGoodpix,
                2,
            )
            self.axes[2].set_title(
                "SPAXEL_ID = {:d}".format(self.idxBinLong), loc="right"
            )
        try:
            line = (
                self.currentMapQuantity.split("_")[0]
                + "_"
                + self.currentMapQuantity.split("_")[1]
            )
            self.axes[2].set_title(
                "Emission-line kinematics: v={:.1f}km/s, sigma={:.1f}km/s".format(
                    self.gasResults[line + "_V"][self.idxBinLong],
                    self.gasResults[line + "_S"][self.idxBinLong],
                ),
                loc="left",
            )
        except:
    ...
    if self.SFH == True:
        ...
        if self.Weights.shape[3] == 2:
            ...
            pcol4 = self.axes[6].pcolormesh(
                self.age,
                self.metals,
                self.Weights[self.idxBinShort, :, :, 1],
                edgecolors="face",
            )
...
def plotSpectraGAS(self, spectra, bestfit, goodpix, panel):
    ...
    if self.gasLevel == "BIN":
        self.axes[panel].plot(
            self.gasLambda,
            self.EmissionSubtractedSpectraBIN[self.idxBinShort, :],
            color="orange",
            linewidth=2,
        )
    elif self.gasLevel == "SPAXEL":
        self.axes[panel].plot(
            self.gasLambda,
            self.EmissionSubtractedSpectraSPAXEL[self.idxBinLong, :],
            color="orange",
            linewidth=2,
        )
...
def plotSpectraSFH(self, spectra, bestfit, goodpix, panel):
    ...
    try:
        ...
        self.axes[panel].plot(
            self.gasLambda[idxLamGand],
            self.EmissionSubtractedSpectraBIN[self.idxBinShort, idxLamGand],
            color="orange",
            linewidth=2,
        )
        self.axes[panel].plot(
...
def plotSFH(self, panel):
    ...
    self.axes[panel].set_title(
        "Star Formation History; Mean Age: {:.2f}".format(
            self.sfhResults["AGE"][self.idxBinLong]
        )
        + " Gyr"
    )
