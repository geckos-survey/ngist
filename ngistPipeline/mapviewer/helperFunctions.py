...
def getVoronoiBin(self):
    ...
    idx = np.where(
        np.logical_and(
            np.abs(self.table.X - self.mouse.xdata) < self.pixelsize,
            np.abs(self.table.Y - self.mouse.ydata) < self.pixelsize,
        )
    )[0]
...
def dialogNotAvailable(self, which):
    ...
    if which == "directory":
        pyqt.QMessageBox.information(
            self,
            "Not available",
            "There is no data available! Please choose another directory! \n:( ",
        )
    else:
        pyqt.QMessageBox.information(
            self, "Not available", which + " data is not available! :( "
        )
...
def selectBinIDfromDialog(self):
    ...
    try:
        ...
        minIdx = np.argmin(
            np.sqrt(
                (self.table.X[allIdx] - self.table.XBIN[allIdx]) ** 2
                + (self.table.Y[allIdx] - self.table.YBIN[allIdx]) ** 2
            )
        )  # Central spaxel of bin
...
def getSettings(self):
    ...
    if (
        self.checkbox_gasLevelBIN.checkState() == 2
        and self.gasLevel != "BIN"
        and self.GAS == True
    ):
        ...
        self.checkbox_gasLevelSPAXEL.checkState() == 2
        and self.gasLevel != "SPAXEL"
        and self.GAS == True
    ):
    ...
    if (
        self.checkbox_LsLevelORIG.checkState() == 2
        and self.LsLevel != "ORIGINAL"
        and self.LINE_STRENGTH == True
    ):
        self.LsLevelSelected = "ORIGINAL"
        self.loadData()
        self.createWindow()
    elif (
        self.checkbox_LsLevelADAP.checkState() == 2
        and self.LsLevel != "ADAPTED"
        and self.LINE_STRENGTH == True
    ):
    ...
    for i in conf.keys():
        ...
        for o, key in enumerate(conf[i]):
            ...
            self.tableWidget.setItem(
                counter, 2, pyqt.QTableWidgetItem(str(conf[i][key]))
            )
            counter += 1
