...
def plotMaps(
    ...
    if INTERACTIVE == True:
        ...
        if flag == "SFH":
            plt.savefig(
                os.path.join(outdir, rootname) + "_sfh.pdf",
                bbox_inches="tight",
                pad_inches=0.3,
            )
        elif flag == "LS":
...
def main(args=None):
    ...
    parser = optparse.OptionParser(
        usage="./utilities/util_plot_spp.py --runname=FCC167_Full-Opt"
    )
    parser.add_option(
        "-f",
        "--flag",
        dest="flag",
        type="string",
        help="Select weather you want to plot SPP maps from unregularized PPXF or LS indices.",
    )
    parser.add_option(
        "-r",
        "--runname",
