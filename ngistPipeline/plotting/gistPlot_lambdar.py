...
def plotMaps(
    ...
    # Read Results
    result = np.array(
        fits.open(os.path.join(outdir, rootname) + "_kin.fits")[1].data.LAMBDA_R
    )

    # Convert results to long version
    result_long = np.zeros(len(binNum_long))
    result_long[:] = np.nan
    ...
    grid[0].text(
        0.985,
        0.008,
        r"{:.2f}".format(vmin).replace("-", r"- ") + r" / " + r"{:.2f}".format(vmax),
    ...
    if INTERACTIVE == True:
        ...
        plt.savefig(
            os.path.join(outdir, rootname) + "_lambda.pdf",
            bbox_inches="tight",
            pad_inches=0.3,
        )
