...
def spatialMasking_Module(config, cube):
    ...
    if os.path.isfile(outputFile) == True and config["GENERAL"]["OW_OUTPUT"] == False:
        logging.info(
            "Results of the spatialMasking module are already in the output directory. Module is skipped."
        )
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            ...
            + "/"
            + config["SPATIAL_MASKING"]["METHOD"]
            + ".py",
        )
        logging.info(
            "Using the spatial masking routine for "
            + config["SPATIAL_MASKING"]["METHOD"]
        )
        ...
        message = (
            "Failed to import the spatial masking routine "
            + config["SPATIAL_MASKING"]["METHOD"]
            + "."
        )
