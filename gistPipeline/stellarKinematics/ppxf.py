...
def saveContsubCube(config, ppxf_bestfit, logLam):
    ...
    printStatus.updateDone(
        "Writing: " + config["GENERAL"]["RUN_ID"] + "_kin-ContSubCube.fits"
    )
    outfits = (
        os.path.join(config["GENERAL"]["OUTPUT"], config["GENERAL"]["RUN_ID"])
        + "_kin-ContSubCube.fits"
    )
    logging.info("Wrote: " + outfits)
    ...
    continuum_cube_hdu = fits.ImageHDU(
        contSubCube.reshape((len(linLam), NY, NX)), name="contsub", header=cubeHeader
    )
    hdul = fits.HDUList([primary_hdu, continuum_cube_hdu])
