...
def workerPPXF(inQueue, outQueue):
    ...
    for templates, bin_data, noise, velscale, start, goodPixels_ppxf, nmoments,\
        adeg, mdeg, reddening, logLam, offset, velscale_ratio, nsims, nbins, i, optimal_template_in \
        in iter(inQueue.get, 'STOP'):

        sol, ppxf_reddening, bestfit, optimal_template, mc_results, formal_error, spectral_mask = \
          run_ppxf(templates, bin_data, noise, velscale, start, goodPixels_ppxf, nmoments, adeg, mdeg, reddening,
          logLam, offset, velscale_ratio, nsims, nbins, i, optimal_template_in)

        outQueue.put(( i, sol, ppxf_reddening, bestfit, optimal_template, mc_results, formal_error, spectral_mask ))


def run_ppxf( templates, log_bin_data, log_bin_error, velscale, start, goodPixels, nmoments, adeg, mdeg, reddening, logLam, \
        offset, velscale_ratio, nsims, nbins, i, optimal_template_in):
    """
    ...
    try:
        ...
        if len(optimal_template_in) == 1:
            ...
            pp = ppxf(templates, log_bin_data, log_bin_error, velscale, start, goodpixels=goodPixels, plot=True, \
        ...
        return(pp.sol[:], pp.reddening, pp.bestfit, optimal_template, mc_results, formal_error, spectral_mask)
...
def save_ppxf(config, ppxf_result, ppxf_reddening, mc_results, formal_error,\
              ppxf_bestfit, logLam, goodPixels, optimal_template, logLam_template, npix, spectral_mask, optimal_template_comb):
    ...
    # Extension 2: Table HDU with logLam_templates
    cols = []
    cols.append( fits.Column(name='OPTIMAL_TEMPLATE_ALL', format='D', array=optimal_template_comb) )
    ...
    outfits = os.path.join(config['GENERAL']['OUTPUT'],config['GENERAL']['RUN_ID'])+'_kin-SpectralMask.fits'
    printStatus.running("Writing: "+config['GENERAL']['RUN_ID']+'_kin-SpectralMask.fits')
    ...
    # Extension 1: Table HDU with optimal templates
    cols = []
    cols.append( fits.Column(name='SPECTRAL_MASK', format=str(spectral_mask.shape[1])+'D', array=spectral_mask ) )
    dataHDU = fits.BinTableHDU.from_columns(fits.ColDefs(cols))
    ...
    saveContsubCube(config, ppxf_bestfit, logLam)
...
def extractStellarKinematics(config):
    ...
    tmp_ppxf_result, tmp_ppxf_reddening, tmp_ppxf_bestfit, optimal_template_out,\
        tmp_mc_results, tmp_formal_error, tmp_spectral_mask = run_ppxf\
        (templates, comb_spec , comb_espec, velscale, start[0,:], goodPixels_ppxf,\
        config['KIN']['MOM'], config['KIN']['ADEG'], config['KIN']['MDEG'], config['KIN']['REDDENING'], \
        logLam, offset, velscale_ratio,nsims, nbins, 0, optimal_template_init)
    ...
    if config['GENERAL']['PARALLEL'] == True:
        ...
        for i in range(nbins):
            inQueue.put( ( templates, bin_data[:,i], noise[:,i], velscale, start[i,:], goodPixels_ppxf,\
                            config['KIN']['MOM'], config['KIN']['ADEG'], config['KIN']['MDEG'], config['KIN']['REDDENING'], \
                            logLam, offset, velscale_ratio, nsims, nbins, i , optimal_template_comb) )
        ...
        #for i in range(1, 2):
            ppxf_result[i,:config['KIN']['MOM']], ppxf_reddening[i], ppxf_bestfit[i,:], optimal_template[i,:],\
              mc_results[i,:config['KIN']['MOM']], formal_error[i,:config['KIN']['MOM']],  spectral_mask[i,:] = run_ppxf\
                (templates, bin_data[:,i], noise[:,i], velscale, start[i,:], goodPixels_ppxf,\
                config['KIN']['MOM'], config['KIN']['ADEG'], config['KIN']['MDEG'], config['KIN']['REDDENING'], logLam, offset, \
                velscale_ratio, nsims, nbins, i,optimal_template_comb)
    ...
    goodPixels_ppxf, optimal_template, logLam_template, npix, spectral_mask, optimal_template_comb)
...
def saveContsubCube(config, ppxf_bestfit, logLam):
    ...
    cube = readCube(config)
    spectra_all = cube['spec']
    linLam = cube['wave']
    idx_lam  = np.where( np.logical_and( linLam > config['KIN']['LMIN'], linLam < config['KIN']['LMAX'] ) )[0]
    ...
    for s in spaxID:
        ...
        contSubCube[:,s] = obsSpec_lin - fitSpec_lin
    ...
    continuum_cube_hdu = fits.ImageHDU(contSubCube.reshape((len(linLam),NY,NX)),
                          name='contsub',
                          header = cubeHeader)
    ...
    # ABW 14.04.23 V1 continuum subtracts in logLam space
    # printStatus.updateDone("Writing: "+config['GENERAL']['RUN_ID']+'_kin-ContSubCube.fits')
    # outfits = os.path.join(config['GENERAL']['OUTPUT'],config['GENERAL']['RUN_ID'])+'_kin-ContSubCube.fits'
    # logging.info("Wrote: "+outfits)
