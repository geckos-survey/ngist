...
def plot_maps(flag, outdir, INTERACTIVE=False, vminmax=np.zeros((4,2)), contour_offset_saved=0.20, SAVE_AFTER_INTERACTIVE=False):
    ...
    # Read results
    if flag == 'SFH':
        sfh_hdu     = fits.open(outdir+rootname+'_sfh.fits')
        ...
        result      = np.zeros((len(ubins),3))
        result[:,0] = np.array( ls_hdu[2].data.AGE[:,50]   )
        result[:,1] = np.array( ls_hdu[2].data.METAL[:,50] )
        result[:,2] = np.array( ls_hdu[2].data.ALPHA[:,50] )
        if "ALPHA" in ls_hdu[2].data.columns.names: 
            labellist = ['AGE', 'METAL', 'ALPHA']
        else:
            labellist = ['AGE', 'METAL']
    ...
    if INTERACTIVE == True:
        ...
        if flag == 'SFH':
            plt.savefig(outdir+rootname+'_spp_ppxf.pdf', bbox_inches='tight', pad_inches=0.3)
        elif flag == 'LS':
            plt.savefig(outdir+rootname+'_spp_ls.pdf', bbox_inches='tight', pad_inches=0.3)
