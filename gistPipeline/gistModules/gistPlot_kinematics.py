...
# Make plotting routines both python 2 and 3 compatible
try:
    input = raw_input
except NameError:
    pass
...
PURPOSE:
  ...
  from both the unregularized (PPXF module) and regularized (SFH module) run of 
  pPXF. 
...
def plot_maps(flag, outdir, INTERACTIVE=False, vminmax=np.zeros((4,2)), contour_offset_saved=0.20, SAVE_AFTER_INTERACTIVE=False):
    ...
    # Read Results
    if flag == 'PPXF':
        hdu = fits.open(outdir+rootname+'_ppxf.fits')
    elif flag == 'SFH':
        hdu = fits.open(outdir+rootname+'_sfh.fits')
    ...
    if INTERACTIVE == True:
        ...
        if inp == 'y' or inp == 'Y' or inp == 'yes' or inp == 'YES':
            ...
            plot_maps(flag, runname, INTERACTIVE=False, vminmax=vminmax, contour_offset_saved=contour_offset, SAVE_AFTER_INTERACTIVE=True)
            ...
            plot_maps(flag, runname, INTERACTIVE=True, vminmax=vminmax, contour_offset_saved=contour_offset, SAVE_AFTER_INTERACTIVE=False)
        else:
        ...
        if flag == 'PPXF':
            plt.savefig(outdir+rootname+'_ppxf.pdf', bbox_inches='tight', pad_inches=0.3)
        elif flag == 'SFH':
            plt.savefig(outdir+rootname+'_sfh_kin.pdf', bbox_inches='tight', pad_inches=0.3)
...
def main(args=None):
    ...
    parser.add_option("-f", "--flag",    dest="flag",    type="string", help="Select weather you want to plot unrgularized (PPXF) or regularized (SFH) kinematic maps.")
    ...
    plot_maps(options.flag, options.runname, INTERACTIVE=True)
