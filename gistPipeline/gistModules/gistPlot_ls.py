...
def plot_maps(outdir, RESOLUTION, INTERACTIVE=False, vminmax=np.zeros((4,2)), contour_offset_saved=0.20, SAVE_AFTER_INTERACTIVE=False):
    ...
    if RESOLUTION == "ORIGINAL": 
        ls_hdu      = fits.open(outdir+rootname+'_ls_OrigRes.fits')
    elif RESOLUTION == "ADAPTED":
        ls_hdu      = fits.open(outdir+rootname+'_ls_AdapRes.fits')
    ...
    grid = AxesGrid(fig, 111, nrows_ncols=(1, 3), axes_pad=0.0, share_all=True,\
    ...
    if INTERACTIVE == True:
        ...
        if inp == 'y' or inp == 'Y' or inp == 'yes' or inp == 'YES':
            ...
            plot_maps(runname, RESOLUTION, INTERACTIVE=False, vminmax=vminmax, contour_offset_saved=contour_offset, SAVE_AFTER_INTERACTIVE=True)
            ...
            plot_maps(runname, RESOLUTION, INTERACTIVE=True, vminmax=vminmax, contour_offset_saved=contour_offset, SAVE_AFTER_INTERACTIVE=False)
        else:
        ...
        if RESOLUTION == "ORIGINAL":
            plt.savefig(outdir+rootname+'_ls_OrigRes.pdf', bbox_inches='tight', pad_inches=0.3)
        elif RESOLUTION == "ADAPTED":
            plt.savefig(outdir+rootname+'_ls_AdapRes.pdf', bbox_inches='tight', pad_inches=0.3)
...
def main(args=None):
    ...
    parser = optparse.OptionParser(usage="./utilities/util_plot_ls.py --runname=FCC167_Full-Opt")
    parser.add_option("-r", "--runname",    dest="runname",    type="string", help="Path to output directory")
    parser.add_option("-s", "--resolution", dest="resolution", type="string", help="Resolution at which the measurement was conducted (ORIGINAL/ADAPTED)")
    (options, args) = parser.parse_args()

    if options.runname[-1] != '/':
        options.runname = options.runname+'/'

    plot_maps(options.runname, options.resolution, INTERACTIVE=True)
