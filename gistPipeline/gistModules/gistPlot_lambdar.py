...
"""
PURPOSE:
  Plot maps of the lambda parameter. 
...
def plot_maps(outdir, INTERACTIVE=False, vminmax=np.array([0.0,1.0]), contour_offset_saved=0.20, SAVE_AFTER_INTERACTIVE=False):
    ...
    # Read Results
    result = np.array( fits.open(outdir+rootname+'_ppxf.fits')[1].data.LAMBDA_R )

    # Convert results to long version
    result_long  = np.zeros( len(binNum_long) ); result_long[:] = np.nan
    ...
    if INTERACTIVE == True:
        ...
        inp = input(" Enter vmin,vmax for "+labellist[0]+\
    ...
    if INTERACTIVE == True:
        ...
        if inp == 'y' or inp == 'Y' or inp == 'yes' or inp == 'YES':
            ...
            plot_maps(runname, INTERACTIVE=False, vminmax=vminmax, contour_offset_saved=contour_offset, SAVE_AFTER_INTERACTIVE=True)
            ...
            plot_maps(runname, INTERACTIVE=True, vminmax=vminmax, contour_offset_saved=contour_offset, SAVE_AFTER_INTERACTIVE=False)
        else:
        ...
        plt.savefig(outdir+rootname+'_lambda.pdf', bbox_inches='tight', pad_inches=0.3)
...
def main(args=None):
    ...
    plot_maps(options.runname, INTERACTIVE=True)
