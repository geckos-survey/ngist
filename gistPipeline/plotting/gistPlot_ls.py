...
def plotMaps(outdir, RESOLUTION, INTERACTIVE=False, vminmax=np.zeros((4,2)), contour_offset_saved=0.20, SAVE_AFTER_INTERACTIVE=False):
    ...
    for iterate in range(0,len(labellist)):
        # Prepare main plot
        val = result[:,iterate]
        ...
        image[i,j] = val
        image_hdu = fits.ImageHDU(image, name=labellist[iterate])
        # Append fits image
        hdu1.append(image_hdu)
    hdu1.writeto(os.path.join(outdir,rootname)+'_lsmaps_' + RESOLUTION + '.fits', overwrite=True)
    #######
    ...
    if INTERACTIVE == True:
        ...
        if inp == 'y' or inp == 'Y' or inp == 'yes' or inp == 'YES':
            ...
            plotMaps(runname, RESOLUTION, INTERACTIVE=True, vminmax=vminmax, contour_offset_saved=contour_offset, SAVE_AFTER_INTERACTIVE=False)
        else:
        ...
        if RESOLUTION == "ORIGINAL":
            plt.savefig(os.path.join(outdir,rootname)+'_ls_OrigRes.pdf', bbox_inches='tight', pad_inches=0.3)
        elif RESOLUTION == "ADAPTED":
            plt.savefig(os.path.join(outdir,rootname)+'_ls_AdapRes.pdf', bbox_inches='tight', pad_inches=0.3)
